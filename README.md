...
- **IANA Bootstrap**：内嵌 RDAP 端点配置开箱即用，启动后异步从 IANA 更新完整配置
...
| `Client.WaitForReady(ctx context.Context) error` | 等待客户端就绪（RDAP Bootstrap 加载完成，通常无需手动调用） |
...
| 函数 | 说明 |
|------|------|
| `DownloadRDAPConfig(bootstrapURL string, destPath string) (map[string]string, error)` | 从 IANA 获取 RDAP Bootstrap 数据，返回 TLD 到端点的映射，destPath 非空时同时保存到本地文件 |
...
当作为第三方库使用时，客户端默认会同步加载内嵌的 RDAP Bootstrap 配置（包含主流 TLD），开箱即用无需额外配置。同时会异步从 IANA 更新完整配置。
...
```go
// 从 IANA 获取 RDAP Bootstrap 数据（仅返回映射）
...
// 获取并保存到本地文件
rdapEndpoints, err = whois.DownloadRDAPConfig("https://data.iana.org/rdap/dns.json", "config/rdap_bootstrap.json")
if err != nil {
    log.Fatal(err)
}
```
